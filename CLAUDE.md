...
4d. **A página final é A4 (595x842 pt), não o tamanho do frame.** O frame do Figma tem 1240x1754 px e o export manda 1 px como 1 pt — isso dava uma página de **43,7 x 61,9 cm**, mais de quatro A4. No celular o visualizador estourava o limite de bitmap e desenhava a página **branca** até o leitor dar zoom. O `arte:preparar` reduz para largura de A4, e por isso `escala: ESCALA_ARTE` (595/1240) em todas as artes — nunca `1`. As coordenadas do config continuam sendo as do painel do Figma: a escala multiplica x, y e o tamanho da fonte no `gerar.ts`. Se a página de destino mudar no script, `ESCALA_ARTE` muda junto — os dois números descrevem a mesma transformação.
4e. A proposta é **100% digital**; ninguém imprime. O alvo de resolução é a tela, não a régua de impressão: 144 DPI (2x a unidade base do PDF) cobre uma página inteira com ~1190 px, mais que a largura da tela de qualquer celular. `arte:preparar -- <arte> <pasta> --dpi N` permite comparar antes de decidir.
