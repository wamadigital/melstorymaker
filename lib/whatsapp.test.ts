...
test("chamar um lead abre a conversa SEM texto pré-escrito", () => {
  ...
  // e quem sabe o que dizer é a Mel.
  const link = linkConversaLead("(19) 99999-8888");
  assert.equal(link, "https://wa.me/5519999998888");
  assert.ok(!link!.includes("?text="), "a caixa de mensagem tem que ir vazia");
});

test("sem telefone não há link — e não pode virar o seletor de conversas da Mel", () => {
  ...
  assert.equal(linkConversaLead(null), null);
  assert.equal(linkConversaLead(""), null);
  assert.equal(linkConversaLead("   "), null);
});
